...
dat2 <- dat[, var_keep]
dat2 <- dat2 %>% mutate(ECOMMERCE=as.numeric((ECOMMERCE==1)))
dat2$ECOMMERCE[is.na(dat2$ECOMMERCE)] <- 0
...
DIRECT <- tidyr::expand_grid(FIPST = FIPST, SECTOR = SECTOR) %>%
  ...
  dplyr::mutate(ECOM = NA) %>%
  dplyr::mutate(ECOM_SE = NA)
...
for (idx1 in 1:dim(DIRECT)[1]) {
  ...
  for (idx2 in 1:10) {
    ...
    ecom_rg[idx2] <- sum(10*tmp_dat$TABWGT[w_rg]*tmp_dat$FPC[w_rg]*tmp_dat$ECOMMERCE[w_rg])
  }
  ...
  DIRECT$ECOM[idx1] <- sum(tmp_dat$TABWGT*tmp_dat$ECOMMERCE)/sum(tmp_dat$TABWGT)
  DIRECT$ECOM_SE[idx1] <- sqrt(sum((ecom_rg - mean(rec_rg))^2/(9*10))*af)
  print(idx1)
}

readr::write_rds(DIRECT, "Data/DIRECT.rds")
